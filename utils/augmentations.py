# adapted from https://github.com/pengsongyou/openscene/blob/main/dataset/augmentation.py
import random
...
class ChromaticAutoContrast(object):
    ...
    def __init__(self, randomize_blend_factor=True, blend_factor=0.4):
...
class ChromaticJitter(object):
    ...
    def __init__(self, std=0.1, mean=0.5):
        self.std = std
        self.mean = mean

    def __call__(self, coords, feats, labels):
        if random.random() < 0.95:
            noise = np.random.randn(feats.shape[0], 3) * self.mean
            noise *= self.std * 255
...
class RandomBlobRemovalPerObj(object):
    def __init__(self, n_blobs_range, blob_size_range):
        '''
        :param n_blobs: number of blobs to remove per object, 
    ...
    def remove_blobs(pointcloud, n_blobs, blob_size):
        ...
        """
        # Ensure blob_size is not larger than a fourth of the total number of points
        blob_size = min(blob_size, len(pointcloud) // 4)
        ...
        for _ in range(n_blobs):
            ...
            blob_center_idx = np.random.randint(
                low=0, high=len(modified_pointcloud))
            ...
            removed_indices.append(blob_indices)

            # Remove the points belonging to the blob
            modified_pointcloud = np.delete(modified_pointcloud, blob_indices, axis=0)
        removed_indices = np.concatenate(removed_indices, axis=0)

        return modified_pointcloud, removed_indices

    def __call__(self, coords, feats, labels):
        objs = np.unique(labels)
        coords_list, feats_list, labels_list = [], [], []
        for obj in objs:
            n_blobs = np.random.randint(
                low=self.n_blobs_range[0], high=self.n_blobs_range[1], size=(1,)).item()
            if not n_blobs:
                coords_list.append(coords[labels == obj])
                feats_list.append(feats[labels == obj])
                labels_list.append(labels[labels == obj])
                continue
            blob_size = np.random.randint(
                low=self.blob_size_range[0], high=self.blob_size_range[1], size=(1,)).item()
            obj_coords = coords[labels == obj]
            _, removed = self.remove_blobs(obj_coords, n_blobs, blob_size)
            coords_list.append(np.delete(obj_coords, removed, axis=0))
            feats_list.append(np.delete(feats[labels == obj], removed, axis=0))
            labels_list.append(np.delete(labels[labels == obj], removed, axis=0))
